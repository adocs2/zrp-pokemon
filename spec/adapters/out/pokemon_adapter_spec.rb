...
describe Pokemon::Adapters::Out::PokemonAdapter do
  context 'when adapting pokemon data' do
    it "return status 200 and pokemon's abilities" do
      params = {
        "abilities": [
          {
            "ability": {
      ...
      expected_result = described_class.adapt_pokemon_info(params)

      expect(expected_result[:status]).to eql(200)
      expect(expected_result[:pokemon_info]).to eql(abilities: %w[imposter limber])
    end

    it 'return 404 and not found when pokemon not exist' do
      params = 'Not Found'

      expected_result = described_class.adapt_pokemon_info(params)

      expect(expected_result[:status]).to eql(404)
      expect(expected_result[:pokemon_info]).to eql('Not Found')
    end
  end
end
